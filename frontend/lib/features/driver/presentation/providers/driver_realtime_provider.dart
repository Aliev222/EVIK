...
class DriverRealTimeNotifier extends StateNotifier<DriverRealTimeState> {
  DriverRealTimeNotifier(this._realTimeService) : super(const DriverRealTimeState()) {
    _initializeServices();
  }
  ...
  Future<void> _sendLocationUpdate() async {
    if (!state.isOnline || !state.isConnected || state.currentLocation == null) {
      return;
    }
    ...
    try {
      ...
      if (state.currentLocation != null) {
        ...
        speed = distance * 720; // Приблизительная скорость (distance за 5 сек * 720 = км/ч)
      ...
      print('📍 Driver location sent: ${location.lat}, ${location.lng}, speed: ${speed.toStringAsFixed(1)} km/h');
...
final driverRealTimeProvider = StateNotifierProvider<DriverRealTimeNotifier, DriverRealTimeState>((ref) {
