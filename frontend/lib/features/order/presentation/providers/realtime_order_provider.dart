...
class RealTimeOrderNotifier extends StateNotifier<AsyncValue<String?>> {
  RealTimeOrderNotifier(this._realTimeService) : super(const AsyncValue.data(null));
  ...
  Future<void> createRealTimeOrder({
    ...
    try {
      ...
      final orderId = 'order_${DateTime.now().millisecondsSinceEpoch}_$clientId';
...
final realTimeOrderProvider = StateNotifierProvider<RealTimeOrderNotifier, AsyncValue<String?>>((ref) {
...
final currentUserIdProvider = Provider<String?>((ref) {
  final user = ref.watch(currentUserProviderNew);
  return user?.id;
});