...
const state = {
  ...
  dashAutoRefresh: null,
  dashAutoRefreshEnabled: true,
  dashRequestId: 0,
  dashRefreshInterval: 60000,
...
const api = {
  ...
  async request(method, path, { body, query, raw, retries, timeout } = {}) {
    ...
    // A lost response does not mean a write failed: repeating a refund or
    // wallet adjustment can apply it twice. Only reads are retried.
    const maxAttempts = ['GET', 'HEAD'].includes(method.toUpperCase())
      ? (retries === undefined ? 2 : retries) : 0;
...
function startDashAutoRefresh() {
  stopDashAutoRefresh();
  if (!state.dashAutoRefreshEnabled || !state.token || state.route.name !== 'dashboard') return;
  state.dashAutoRefresh = setInterval(() => {
    if (state.token && state.route.name === 'dashboard' && !document.hidden) {
      const main = document.querySelector('.main-content');
      if (main) pageDashboard(main);
    }
  }, state.dashRefreshInterval);
}

function stopDashAutoRefresh() {
  if (state.dashAutoRefresh) {
    clearInterval(state.dashAutoRefresh);
    state.dashAutoRefresh = null;
  }
}
...
function scheduleWsReconnect() {
  ...
  if (!state.token || state.route.name !== 'dashboard') return;
...
async function pageDashboard(main) {
  const requestId = ++state.dashRequestId;
  const isCurrent = () => requestId === state.dashRequestId && main.isConnected && state.token && state.route.name === 'dashboard';
  ...
  const autoRefreshOn = state.dashAutoRefreshEnabled;
      ...
      <section class="dash-charts">
        <div class="dash-card">
          <div class="dash-card-head">
            ...
            <div class="dash-chart-controls">
              ...
              <div class="dash-card-figure">${formatMoneyMinor((gmvFiltered || []).reduce((sum, p) => sum + (Number(p.amount) || 0), 0))}</div>
            </div>
          </div>
        ...
        <div class="dash-card">
          <div class="dash-card-head">
            ...
            <div class="dash-card-figure">${formatMoneyMinor((commFiltered || []).reduce((sum, p) => sum + (Number(p.amount) || 0), 0))}</div>
          </div>
  ...
  if (autoRefreshBtn) {
    autoRefreshBtn.onclick = () => {
      state.dashAutoRefreshEnabled = !state.dashAutoRefreshEnabled;
      if (!state.dashAutoRefreshEnabled) {
        stopDashAutoRefresh();
      } else {
        startDashAutoRefresh();
      }
...
async function pageDrivers(main) {
  ...
  if (routeParams.driverId) {
    return pageDriverDetail(main, routeParams.driverId);
  }
...
function renderApp() {
  ...
  // Stop dashboard auto-refresh when navigating to any page
  if (state.route.name !== 'dashboard') { stopDashAutoRefresh(); disconnectWs(); }
  renderShell();
...
function renderShell() {
  ...
  // The shell is replaced on navigation; keep this listener on that shell
  // so old theme menus are not retained by document-level closures.
  $('#app').onclick = (e) => {
