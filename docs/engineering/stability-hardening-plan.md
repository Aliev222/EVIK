...
| STAB-010 | Dependency security | Govulncheck обнаружил достижимые уязвимости в Go toolchain `1.26.5`, `grpc v1.81.1` и `x/text v0.37.0`. | Official `govulncheck`: 9 findings initially; after `GOTOOLCHAIN=go1.26.6` and dependency upgrade: `No vulnerabilities found`, 0 reachable. | High (resolved) | HTTP/TLS/XML/normalization attack surface and gRPC affected. | `GOTOOLCHAIN=go1.26.6 go run golang.org/x/vuln/cmd/govulncheck@latest ./...` | Pin Go `1.26.6`; upgrade reachable dependencies (`grpc v1.82.1`, `x/text v0.39.0`, compatible transitive modules). | DONE | Pending | `go test`, `go vet`, `go build`, `govulncheck` PASS. Govulncheck still reports non-reachable findings in imported/required modules; they need periodic review. |
| STAB-011 | Concurrency / dispatch tests | `go test -race ./...` выявил races в fake dispatch repository/global test state при фоне `DriverBecameAvailable`. | Race detector traces pointed to unsynchronized `fakeOfferRepo.created` reads and global `allOrders` while scheduler goroutine ran. | Medium (resolved) | Race gate был красный и скрывал возможность подтвердить runtime concurrency. | `GOTOOLCHAIN=go1.26.6 go test -race ./...` (до: FAIL, после: PASS). | Add mutex-protected snapshots/setter in test double; production scheduler behavior unchanged. | DONE | Pending | Full `go test -race ./...` PASS after fix. This validates current tests, not multi-instance production behavior. |
