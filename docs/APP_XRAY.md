...
Граф: индекс GitNexus для репозитория EVIK. Он должен обновляться перед анализом существенных изменений.
Метод: GitNexus + фактический код, тесты и схема БД. Код не менялся.
...
- GitNexus — основной инструмент навигации по коду и анализа влияния изменений. Перед существенными правками нужно проверить свежесть индекса; перед коммитом — проанализировать изменённый граф.
