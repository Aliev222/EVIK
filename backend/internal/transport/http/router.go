...
func NewRouter(
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(secured chi.Router) {
			...
			secured.With(RequireRoles(auth.RoleClient, auth.RoleDriver, auth.RoleAdmin)).Get("/orders/{orderID}/receipt", paymentHandler.GetOrderReceipt)
			...
			secured.With(RequireRoles(auth.RoleDriver, auth.RoleAdmin)).Get("/driver/wallet/debt", paymentHandler.ListDriverDebt)
